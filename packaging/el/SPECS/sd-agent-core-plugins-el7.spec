...
for plugin in activemq_xml agent_metrics apache btrfs cacti cassandra ceph consul couch couchbase directory disk dns_check docker_daemon elastic etcd fluentd gearmand gunicorn haproxy hdfs_datanode hdfs_namenode http_check jenkins kafka kafka_consumer kong kube_dns kubernetes kubernetes_state kyototycoon lighttpd linux_proc_extras mapreduce marathon mcache mongo mysql network nginx ntp openstack pgbouncer php_fpm postfix postgres powerdns_recursor process rabbitmq redisdb riak riakcs snmp solr spark ssh_check statsd supervisord system_core system_swap tcp_check tokumx tomcat twemproxy varnish vsphere yarn zk; do
    echo ${plugin}
